...
static unique_ptr<FunctionData> QuackActiveConnectionsBind(ClientContext &, TableFunctionBindInput &,
	...
	names = {"server_id", "connection_id", "query", "state", "query_started_at", "last_activity_at", "result_ready_at"};
...
static void QuackActiveConnectionsScan(ClientContext &context, TableFunctionInput &input, DataChunk &output) {
	...
	for (auto &snap : snapshots) {
		...
		output.SetValue(4, row,
		                snap.query_state == QuackQueryState::IDLE ? Value(LogicalType::TIMESTAMP)
		                                                          : Value::TIMESTAMP(snap.query_started_at));
		output.SetValue(5, row,
		                snap.last_activity_at.value == 0 ? Value(LogicalType::TIMESTAMP)
		                                                 : Value::TIMESTAMP(snap.last_activity_at));
		output.SetValue(6, row,
		                snap.result_ready_at.value == 0 ? Value(LogicalType::TIMESTAMP)
		                                                : Value::TIMESTAMP(snap.result_ready_at));
		row++;
	}
